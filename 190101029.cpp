...
string toNum(int num, int width, char ex = '0'){
	stringstream temp;
	temp <<  std::hex << std::uppercase << std::setfill(ex) << std::setw(width) << num;
	return temp.str();
}

// similar to format_name used last time
string toString(string name, int width, char ex = ' '){
	stringstream temp;
	temp << std::left << std::setfill(ex) << std::setw(width) << name;
	return temp.str();
}
...
struct ExpEvaluator{
	...
	pll apply_operator(pll a, pll b, string op){
	...
	vector<string> changeToToken(string exp){
		int n = exp.length();
		...
		while(i < n){
			...
			if(chkSpecial(exp[i])){
				...
				int j = i;
				// accumulate consecutive non special characters
				while(j < n && chkSpecial(exp[j]) == false) j++;
	...
	pll expValue(string exp){
		vector<string> tokens = changeToToken(exp);
		stack<pll> operands;
		stack<int> type;
		...
		int n = tokens.size();
		for(int i = 0; i < n; i++){
			if(tokens[i] == "(") operators.push("(");
			else if(checkNum(tokens[i])) operands.push(make_pair(stoi(tokens[i]), stoi(tokens[i])));
			else if(tokens[i] == ")"){
				while(!operators.empty() && operators.top() != "("){
					pll operand2 = operands.top();
					operands.pop();
					pll operand1 = operands.top();
					operands.pop();
					string op = operators.top();
					operators.pop();
					operands.push(apply_operator(operand1, operand2, op));
				}
				if(!operators.empty()) operators.pop();
			}
			else if(chkSpecialS(tokens[i])){
...
struct instruction{
	...
	void operand_change(int& operand){
		if(length == 3 || length == 4){
			// apply mask for negative operand
			masking(operand, (length == 3) ? 12 : 20);
			if(!e){
				// positions 13, 14, 15
				operand |= (p << 13);
				operand |= (b << 14);
...
// takes a line from source code/intermediate code and changeToToken the line
int process_line(string line, instruction& instr, bool input = true){
	...
	if(!input){
		...
		if(!LOCCTR.empty()) return stoi(LOCCTR, nullptr, 16);
	}
	return -1;
}
...
void pass_1(string input){
	fstream fin, fout;
	// open input file passed by command line
	...
	while(fin.good()){
		...
		if(instr.label != "."){
			if(instr.opcode == "START" || instr.opcode == "CSECT"){
				...
				if(instr.operands != "") STADDR = stoi(instr.operands, NULL, 16);
				else STADDR = 0;
				...
				fout << toString(toNum(LOCCTR, 4), FIELD_LENGTH) << line << '\n';
			}
			else if(instr.opcode == "END" || instr.opcode == "LTORG"){
				// write to intermediate file
				fout << toString("", FIELD_LENGTH) << line << '\n';
				// Update LITTAB
				for(auto& x : LITTAB){
					if(x.second.value == -1){
						// write to intermediate file
						fout << toString(toNum(LOCCTR, 4), FIELD_LENGTH) << toString("*", FIELD_LENGTH) << "=" << toString(x.first.name, FIELD_LENGTH) << " " << toString("", OPERAND_LENGTH) << '\n';

						// Update literal address
						x.second.value = LOCCTR;

						// Update LOCCTR according to the literal type
						if(x.first.name.front() == 'C') LOCCTR += (x.first.name.length() - 3);
						else if(x.first.name.front() == 'X') LOCCTR += ((x.first.name.length() - 3)/2);
						else LOCCTR += 3;
					}
				}
				...
				if(instr.label!=""){
					if(SYMTAB.find(SYMBOL(CSECT, instr.label)) == SYMTAB.end()){
						if(instr.operands == "*"){
						...
						else if(checkNum(instr.operands)){ // assumption that number is in decimal format only
							SYMTAB.insert({SYMBOL(CSECT, instr.label), VALUE(stoi(instr.operands), 3, 0)});
							// write to intermediate file
							fout << toString(toNum(stoi(instr.operands), 4), FIELD_LENGTH) << line << '\n';
						}
						else{
							// Evaluate the expression
							ExpEvaluator e(CSECT);
							int value = e.expValue(instr.operands).second;
							int type = e.expValue(instr.operands).first;
							...
							masking(value, 16);
							// write to intermediate file
							fout << toString(toNum(value, 4), FIELD_LENGTH) << line << '\n';
						}
					}
					else{
				...
				// Update SYMTAB
				if(instr.label!=""){
					if(SYMTAB.find(SYMBOL(CSECT, instr.label)) == SYMTAB.end()) SYMTAB.insert({SYMBOL(CSECT, instr.label), VALUE(LOCCTR)});
					else{
						perror("duplicate symbol");
						exit(1);
					}
				...
				if(OPTAB(instr.opcode) != make_pair(-1, -1)){
					pll format = OPTAB(instr.opcode);
					// extended instruction format
					if(instr.e) format.first++;
					instr.length = format.first;	
					LOCCTR += instr.length;
				}
				else if(instr.opcode == "WORD") LOCCTR += 3;
				else if(instr.opcode == "RESW") LOCCTR += 3 * stoi(instr.operands);
				else if(instr.opcode == "RESB") LOCCTR += stoi(instr.operands);
				else if(instr.opcode == "BYTE"){
					if(instr.operands.front() == 'C') LOCCTR += (instr.operands.length() - 3);
					if(instr.operands.front() == 'X') LOCCTR += ((instr.operands.length() - 3)/2);
				}	
				else{
...
void pass_2(){
	...
	string CSECT = "", PROGNAME = "", text = "", tmp = "";


	// for record storing
	map<string, vector<string>> text_list, modification_list;
	map<string, string> hlist, elist, define_list, rlist;
	...
	while(fin.good()){
		...
		if(instr.label != "."){
			if(instr.opcode == "START"){
				...
				stringstream header;
				header << "H^";
				header << toString(CSECT, 6);
				header << "^" + toNum(STADDR, 6);
				header << "^" + toNum(LENGTH, 6);
				hlist.insert({CSECT, header.str()});
			}
				...
				elist.insert({CSECT, end.str()});

				end << "^" + toNum(first, 6);
				elist[PROGNAME] =  end.str();
			}
				...
				if(text.length()){
					...
					text_record << "T";
					text_record << "^" + toNum(START, 6);
					text_record << "^" + toNum(LENGTH, 2);
					text_record << tmp;
				...
				int LENGTH = 0; 
				if(SYMTAB.find(SYMBOL(CSECT, CSECT)) != SYMTAB.end()) LENGTH = SYMTAB[SYMBOL(CSECT, CSECT)].length;
				...
				if(instr.opcode == "EXTDEF"){
					...
					for(string operand: operands){
						def << "^" + toString(operand, 6);
						int address = 0;
						if(SYMTAB.find(SYMBOL(CSECT, operand)) != SYMTAB.end()) address = SYMTAB[SYMBOL(CSECT, operand)].value;
						def << "^" + toNum(address, 6); 
					}
					...
					EXTREF.clear();
					for(auto x : operands) EXTREF.insert(x);
					stringstream refer;
					refer << "R";
					for(string operand : operands) refer << "^" + toString(operand, 6);
					rlist.insert({CSECT, refer.str()});
				}
			}
			else if(instr.opcode == "BASE"){
				// write listing for the instruction
				...
				if(SYMTAB.find(SYMBOL(CSECT, instr.operands)) != SYMTAB.end()) BASE = SYMTAB[SYMBOL(CSECT, instr.operands)].value;
			}
				...
				if(OPTAB(instr.opcode) != make_pair(-1, -1)){
					...
					if(instr.operands != ""){
						...
						if(instr.operands.find(",") != string::npos){
							...
							// instructions of type BUFFER,X
								...
								if(SYMTAB.find(SYMBOL(CSECT, m_operands.front())) != SYMTAB.end()){
									...
									if(instr.p){
										...
										if(operandcode - PC >= -2048 && operandcode - PC <= 2047) operandcode -= PC;
									...
									if(instr.e){
										...
										modification << "M";
										modification << "^" + toNum(LOCCTR + 1, 6);
										modification << "^05^+";
										modification << toString(CSECT, 6); 
									...
									if(EXTREF.find(m_operands.front()) != EXTREF.end()){
										...
										modification << "^" + toNum(LOCCTR + 1, 6);
										modification << "^05^+";
										modification << toString(m_operands.front(), 6); 
										modification_list[CSECT].push_back(modification.str());
									}
								}
								instr.x = 1;
							...
							// if operand is immediate or external reference
								...
								if(EXTREF.find(instr.operands) != EXTREF.end()){
									...
									modification << "^" + toNum(LOCCTR + 1, 6);
									modification << "^05^+";
									modification << toString(instr.operands, 6); 
									modification_list[CSECT].push_back(modification.str());
								}
								if(instr.i){
									if(checkNum(instr.operands))
						...
						instr.operand_change(operandcode);	
					}
					
					// writing listing for the instruction
					obcode = toNum(op, 2) + toNum(operandcode, 2 * (instr.length - 1));
					fout2 << line << toString(obcode, 10) << '\n';	
					...
					if(instr.label == "*"){
						...
						if(instr.opcode.front() == 'X'){
							...
							obcode = toNum(x, constant.length());
						}
						else if(instr.opcode.front() == 'C'){
							for(char ch : constant){
								int x = ch;
								obcode += toNum(x, 2, false);
							}
						}
						else{
							int word = stoi(instr.operands);
							obcode = toNum(word, 6, false);
						}
					}
					...
					if(instr.opcode == "WORD"){
						if(checkNum(instr.operands)){
							...
							ExpEvaluator e(CSECT);
							int word = e.expValue(instr.operands).second; 
							masking(word, 24);
							obcode = toNum(word, 6);
							vector<string> m_operands = e.changeToToken(instr.operands);
							for(int i = 0; i < (int)(m_operands.size()); i++){
								if(EXTREF.find(m_operands[i]) != EXTREF.end()){
									stringstream modification;
									modification << "M";
									modification << "^" + toNum(LOCCTR, 6);
									modification << "^06";
									if(i > 0 && m_operands[i-1] == "-") modification << "^-";
									else modification << "^+";
									modification << toString(m_operands[i], 6); 
									modification_list[CSECT].push_back(modification.str());
								}
							}
						}
	...
	for(string CSECT: CSECTS){
		fout << hlist[CSECT] << '\n';
		if(!define_list[CSECT].empty()) fout << define_list[CSECT] << '\n';
		if(!rlist[CSECT].empty()) fout << rlist[CSECT] << '\n';
...
int main(int argc, char **argv){
	...
	if(argc != 2){
		cout << "For compilation:\n";
		cout << "g++ -g ./190101029.exe 190101029.cpp\n\n";
		cout << "Format for running after compilation is done:\n";
		cout << "For windows user ->  ./190101029.exe input.txt\n";
		return 0;
	}
	string input = argv[1];
	pass_1(input);
